 ...
 */
export function extractModules(schema: any): string[] {
 ...
 * 获取指定模块的所有端点
 */
export function getEndpointsByModule(schema: any, moduleName: string): Endpoint[] {
  ...
  for (const path in schema.paths) {
    ...
    for (const method in pathItem) {
      if (['get', 'post', 'put', 'delete', 'patch'].includes(method)) {
        ...
        if (
          // 如果标签中包含模块名
          tags.map((t: string) => t.toLowerCase()).includes(moduleName.toLowerCase()) ||
          // 或者路径的第一级与模块名匹配
