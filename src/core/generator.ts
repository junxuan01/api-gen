...
import { ApiGenError } from '../utils/errorHandler.js';

/**
 * 解析OpenAPI schema文件（支持JSON和YAML格式）
 */
...
export async function generateApiCode(
  ...
  // 解析Schema文件（支持JSON和YAML）
  let schema;
  try {
    schema = await parseSchemaFile(schemaPath);
  } catch (error: any) {
    throw new ApiGenError(
      `无法读取或解析Schema文件: ${schemaPath}`, 
      error.message
    );
  }
  ...
  try {
    ...
    typesOutput = typeof result === 'string' ? result : JSON.stringify(result);
    
    // 格式化类型代码（如果启用）
    if (options.format !== false) {
      typesOutput = await formatCode(typesOutput);
    }
  } catch (error) {
...
export interface operations {
  ...
  await fs.writeFile(
    axiosFilePath, 
    options.format !== false ? await formatCode(axiosInstanceCode) : axiosInstanceCode, 
    'utf-8'
  );
  ...
  for (const moduleName of modules) {
    ...
    for (const endpoint of endpoints) {
      const { functionName, code } = generateApiFunction(endpoint, service.name, options.prefix);
      
      const apiFilePath = path.join(moduleDir, `${functionName}.ts`);
      await fs.writeFile(
        apiFilePath, 
        options.format !== false ? await formatCode(code) : code, 
        'utf-8'
      );
    ...
    await fs.writeFile(
      moduleIndexPath, 
      options.format !== false ? await formatCode(moduleIndexCode) : moduleIndexCode,
      'utf-8'
    );
  }
  ...
  await fs.writeFile(
    indexPath, 
    options.format !== false ? await formatCode(indexCode) : indexCode,
    'utf-8'
  );
