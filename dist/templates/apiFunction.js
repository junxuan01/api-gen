 ...
 * 生成API请求函数
 */
export function generateApiFunction(endpoint, serviceName) {
    const { path, method, operationId, summary, parameters, requestBody, responses } = endpoint;
    // 生成函数名
    const functionName = operationId ? camelCase(operationId) : camelCase(`${method}${path.split('/').filter(Boolean).map(pascalCase).join('')}`);
    // 确定参数类型
    let hasParams = false;
    let hasQuery = false;
    let hasBody = false;
 ...
 */
function determineTypeFromSchema(schema) {
    if (!schema)
        return 'any';
    switch (schema.type) {
        case 'string':
